...
def losst(currindex, t, rloss, w, B, mu=0.2, lamb=[0.01, 0.01, 0.01], U=[13], pi=list(range(len(envs)))):
    ...
    points = B[t]
    sim = [sum(nn.CosineSimilarity()(pfs[t].state_dict()['last.weight'].view(-1, 1),
                                     pfs[i].state_dict()['last.weight'].view(-1, 1))) for i in range(len(envs))]
    sim[t] = sim[t] + 100
    rlossRank_renew = 1 + len(envs) - rankdata(sim, method='min')
    term0 = (1 + mu * sum([torch.norm(torch.tensor(B[t][i]), p=1) for i in set(list(range(len(envs)))) - set([t])])) * \
            rloss[t]
    term1 = sum([sum([sum([torch.norm(
        w[i][s] - sum([B[pi[j]][s] * w[i][pi[j]] for j in range(currindex - 1)]) - B[t][s] * w[i][t], p=2) ** 2]) for i
                      in range(len(pfs[0].state_dict().keys()))]) for s in U])
    term2 = torch.norm(
        torch.tensor(priors[current]) - torch.tensor([pss(torch.tensor(i - 0.01), points) for i in points])) ** 2
    term3 = torch.norm(
        torch.tensor(rlossRank) - torch.tensor([pss(torch.tensor(i - 0.01), points) for i in points])) ** 2
    term4 = torch.norm(
        torch.tensor(rlossRank2) - torch.tensor([pss(torch.tensor(i - 0.01), points) for i in points])) ** 2
    ...
    if len(terms_history[0]) <= 1:
        ...
        return (1 / (np.array(terms_history[0]).std()) ** 2) * term0 + (
                    1 / (np.array(terms_history[1]).std()) ** 2) * term1 + (
                           1 / (np.array(terms_history[2]).std()) ** 2) * term2 + (
                           1 / (np.array(terms_history[3]).std()) ** 2) * term3 + (
                           1 / (np.array(terms_history[4]).std()) ** 2) * term4 + np.log(
            (np.array(terms_history[0]).std()) * (np.array(terms_history[1]).std()) * (
                np.array(terms_history[2]).std()) * (np.array(terms_history[3]).std()) * (
                np.array(terms_history[4]).std()))
...
def lossw(currindex, t, rloss, w, B, mu=0.2, lamb=[0.01, 0.01, 0.01], U=[13], pi=list(range(len(envs)))):
    ...
    term1 = sum(
        [sum([torch.norm(w[i][t] - sum([B[pi[j]][t] * w[i][pi[j]] for j in range(currindex - 1)]), p=2) ** 2]) for i in
         range(len(pfs[0].state_dict().keys()))])
    term2 = torch.norm(
        torch.tensor(priors[current]) - torch.tensor([pss(torch.tensor(i - 0.01), points) for i in points])) ** 2
    term3 = torch.norm(
        torch.tensor(rlossRank) - torch.tensor([pss(torch.tensor(i - 0.01), points) for i in points])) ** 2
...
def optimize(loss_func, constraint, optimizer_class, iterations=100):
    ...
    iterates = [[B[t][i].data if i! = t else B[t][i]
    for i in range(len(envs))]]
    losses = []
    ...
    for _ in range(iterations):
        ...
        losses.append(loss)
        iterates.append([B[t][i].data if i! = t else B[t][i]
        for i in range(len(envs))])
    loss = loss_func(currindex, t, rloss, w, B, U=list(set(U) - set(list([t])))).detach()
    losses.append(loss)
    B[t] = [B[t][i].data if i! = t else B[t][i]
    for i in range(len(envs))]
